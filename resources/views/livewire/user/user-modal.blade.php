 <x-modal-card id="user-modal" title="{{ $form->userId ? 'Edit User' : 'Add User' }}" name="userModal">
     <form id="userForm" wire:submit="save" class="space-y-4">
         <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
             <x-input label="First Name" placeholder="First Name" name="form.first_name"
                 wire:model.defer="form.first_name" />
             <x-input label="Last Name" placeholder="Last Name" name="form.last_name"
                 wire:model.defer="form.last_name" />
         </div>
         <x-input label="Email" placeholder="Email" name="form.email" wire:model="form.email" />
         <x-select label="Roles" placeholder="Select user roles" :options="$roles" option-label="name"
             option-value="id" wire:model.live="userRoleIds" name='form.userRoleIds' multiselect />
